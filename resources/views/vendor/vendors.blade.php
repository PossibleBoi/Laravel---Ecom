            ...
            <div class="wrapper">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="content-header">
                        ...
                        <table class="table table-dark table-hover" border=1>
                            ...
                            @foreach ($users as $user)
                                <tr>
                                    ...
                                    <td>
                                            @php
                                                $teamId = $user->roles->first()->pivot->team_id;
                                                $team = \App\Models\Team::where('id', $teamId);
                                            @endphp
                                            {{ $team->first()->display_name }}
                                    </td>
                                    <td>{{$team->first()->description}}</td>
                                    <td>{{ $user->created_at }}</td>
                                    <td>
                                        <a href="{{ route('vendor.edit', ['user' => $user]) }}">Edit</a>
                        ...
                        <p><a href="{{route('team.add')}}">Add A Team</a></p>
                        <table class="table table-dark table-hover" border=1>
                            <tr>
                                <th>Team</th>
                                <th>Team Description</th>
                            ...
                            @foreach ($teams as $team)
                                    ...
                                    <td>
                                        <a href="{{ route('team.edit', ['team' => $team]) }}">Edit</a>

                                        <form method='POST' action="{{ route('team.delete', ['team' => $team]) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button>Delete</button>
                                        </form>
